...
describe('Void Attention Mediator', () => {
  ...
  test('PD: 200 rounds', async () => {
    ...
    const coopCount = result.rounds.filter(
      (r) => r.offerA === 1 && r.offerB === 1
    ).length;
...
describe('Head-to-Head: Three-Walker vs Void Attention', () => {
  ...
  for (const [name, payoff, n] of games) {
    test(`${name}: 500 rounds, 5 seeds`, async () => {
      ...
      const threeWalkerResults: {
        coordRate: number;
        avgPayA: number;
        avgPayB: number;
        settled: boolean;
      }[] = [];
      const voidAttnResults: {
        coordRate: number;
        avgPayA: number;
        avgPayB: number;
        settled: boolean;
      ...
      for (const seed of seeds) {
        ...
        const twCoord =
          tw.rounds.filter((r) => r.offerA === r.offerB).length /
          tw.rounds.length;
        const twPayA =
          tw.rounds.reduce((s, r) => s + r.payoffA, 0) / tw.rounds.length;
        const twPayB =
          tw.rounds.reduce((s, r) => s + r.payoffB, 0) / tw.rounds.length;
        threeWalkerResults.push({
          coordRate: twCoord,
          avgPayA: twPayA,
          avgPayB: twPayB,
          settled: tw.settled,
        ...
        const vaCoord =
          va.rounds.filter((r) => r.offerA === r.offerB).length /
          va.rounds.length;
        const vaPayA =
          va.rounds.reduce((s, r) => s + r.payoffA, 0) / va.rounds.length;
        const vaPayB =
          va.rounds.reduce((s, r) => s + r.payoffB, 0) / va.rounds.length;
        voidAttnResults.push({
          coordRate: vaCoord,
          avgPayA: vaPayA,
          avgPayB: vaPayB,
          settled: va.settled,
      ...
      const twAvgCoord =
        threeWalkerResults.reduce((s, r) => s + r.coordRate, 0) / seeds.length;
      const vaAvgCoord =
        voidAttnResults.reduce((s, r) => s + r.coordRate, 0) / seeds.length;
      const twAvgPay =
        threeWalkerResults.reduce((s, r) => s + r.avgPayA + r.avgPayB, 0) /
        (seeds.length * 2);
      const vaAvgPay =
        voidAttnResults.reduce((s, r) => s + r.avgPayA + r.avgPayB, 0) /
        (seeds.length * 2);
      ...
      console.log(
        `  Three-Walker: coord=${(twAvgCoord * 100).toFixed(
          1
        )}%, avgPay=${twAvgPay.toFixed(2)}, settled=${twSettled}/5`
      );
      console.log(
        `  Void Attn:    coord=${(vaAvgCoord * 100).toFixed(
          1
        )}%, avgPay=${vaAvgPay.toFixed(2)}, settled=${vaSettled}/5`
      );
      console.log(
        `  Delta:        coord=${((vaAvgCoord - twAvgCoord) * 100).toFixed(
          1
        )}pp, pay=${(vaAvgPay - twAvgPay).toFixed(2)}`
      );
...
describe('Chester v Maxell: Bazaar vs Three-Walker vs Void Attention', () => {
  test('head-to-head across 5 seeds', async () => {
    ...
    console.log(
      '  Seed  | Bazaar              | Three-Walker         | Void Attention'
    );
    console.log(
      '  ------|---------------------|----------------------|----------------------'
    );
    ...
    for (const seed of seeds) {
      ...
      const bzAvg =
        bz.rounds.reduce((s, r) => s + r.maxellPayoff + r.chesterPayoff, 0) /
        (bz.rounds.length * 2);
      ...
      console.log(
        `  ${seed.toString().padEnd(5)} | ${(
          bzDesc + ` pay=${bzAvg.toFixed(1)}`
        ).padEnd(19)} | ${(twDesc + ` pay=${twAvg.toFixed(1)}`).padEnd(
          20
        )} | ${vaDesc} pay=${vaAvg.toFixed(1)}`
      );
    }
    ...
    console.log(
      `  Avg   | settled=${bzSettled}/5 pay=${bzAvgPay.toFixed(
        1
      )} | settled=${twSettled}/5 pay=${twAvgPay.toFixed(
        1
      )}  | settled=${vaSettled}/5 pay=${vaAvgPay.toFixed(1)}`
    );
    console.log(
      `  Improvement over Three-Walker: ${(
        ((vaAvgPay - twAvgPay) / Math.abs(twAvgPay)) *
        100
      ).toFixed(1)}% avg payoff`
    );
